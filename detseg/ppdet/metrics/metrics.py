...
class COCOMetric(Metric):
    ...
    def reset(self):
        ...
        self.results = {'bbox': [], 'mask': [], 'segm': [], 'keypoint': [], 'segmentation': []}
        self.eval_results = {}
    ...
    def update(self, inputs, outputs):
        ...
        if('logits' in outputs and 'ori_shape' in outputs):
            segs = []
            logits = outputs['logits'].numpy().astype(np.uint8)
            ori_shapes = outputs['ori_shape'].numpy()
            for logit, ori_shape, cur_im_id in zip(logits, ori_shapes, outs['im_id']):
                h, w = logit.shape[-2:]
                ...
                for cid in range(8, 11):
                    ...
                    for contour in contours:
                        ...
                        if(len(points) >= 3):
                            ...
                            segs.append( {'category_id': int(cid),
                                          'count': int(area),
                                          'image_id': int(cur_im_id),
                                          'segmentation': points } )
            self.results['segmentation'] += segs
    ...
    def accumulate(self):
        ...
        if len(self.results['segmentation']) > 0:
            output = 'seg.json'
            with open(output, 'w') as f:
                json.dump(self.results['segmentation'], f)
                logger.info('The segmentation result is saved to seg.json')
