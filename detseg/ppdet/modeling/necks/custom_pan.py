...
class CustomCSPPAN(nn.Layer):
    ...
    def __init__(self,
        ...
        #self.fix_shapes = [[23, 40], [45, 80], [90, 160]]
    ...
    def forward(self, blocks, for_mot=False):
        ...
        for i, block in enumerate(blocks):
            ...
            if i < self.num_blocks - 1:
                size = blocks[i+1].shape[-2:]
                route = self.fpn_routes[i](route)
                #route = F.interpolate(
                #    route, scale_factor=2., data_format=self.data_format)
                #route = F.interpolate(route, size=self.fix_shapes[i+1], data_format=self.data_format)
                route = F.interpolate(route, size=size, data_format=self.data_format)
