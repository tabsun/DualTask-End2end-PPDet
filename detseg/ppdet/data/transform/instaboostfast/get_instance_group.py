...
def get_mask(bbox, size):
    w, h = size
    xmin, ymin, xmax, ymax = [int(round(x)) for x in bbox]
    retMat = np.zeros((h,w,1), dtype=np.uint8)
    retMat[ymin:ymax, xmin:xmax, :] = 1
    return retMat
...
def extract_single_instance(ann, img, flip_prob=0):
    """
    ...
    x, y , w, h = [float(elem) for elem in ann['bbox']]
    xmin, ymin, xmax, ymax = x, y, x+w, y+h
    normed_xmin = xmin / width
    normed_ymin = ymin / height
    normed_xmax = xmax / width
    normed_ymax = ymax / height

    mask = cocoseg_to_binary(ann['segmentation'], height, width)  # zero one mask
    alpha = mask.astype(np.int32) * 255

    ymin, ymax, xmin, xmax = [int(round(x)) for x in (ymin, ymax, xmin, xmax)]
    result = np.dstack((img[ymin:ymax, xmin:xmax], alpha[ymin:ymax, xmin:xmax]))
    if(flip_prob > 0 and random.random() < flip_prob):
        result = np.flip(result, axis=1)

    return result, [normed_xmin, normed_ymin, normed_xmax, normed_ymax]

def extract(anns: list, img: np.ndarray, configs: list):
    """
    Inpaint background, extract list of instances (in overlapping groups), get transformations and other useful
    information from input image and annotations.
    :param anns: list of input annotations
    ...
    for i in range(len(anns)):
        ...
        resulti, transformi = get_transform(resulti, restricts, configs[i]) # resulti may be flipped
    ...
    return background, instances_list, transforms_list, groupbnd_list
