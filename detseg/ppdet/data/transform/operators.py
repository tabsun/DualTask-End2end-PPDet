...
class Resize(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        # resize mask
        if 'mask' in sample:
            mask = cv2.resize(sample['mask'].astype(np.uint8),
                              None,
                              None,
                              fx=im_scale_x, 
                              fy=im_scale_y,
                              interpolation=cv2.INTER_CUBIC)
            sample['mask'] = np.clip(mask, 0, 3)
...
class RandomCrop(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        for thresh in thresholds:
            ...
            if found:
                ...
                if('mask' in sample):
                    sample['mask'] = self._crop_image(sample['mask'], crop_box)
    ...
    def _crop_image(self, img, crop):
        ...
        if(len(img.shape) == 3):
            ...
            raise "_crop_image gets unknown image shape"
...
class Pad(BaseOperator):
    ...
    def apply_mask(self, image, offsets, im_size, size):
        x, y = offsets
        im_h, im_w = im_size
        h, w = size
    ...
    def apply(self, sample, context=None):
        ...
        if 'mask' in sample:
            sample['mask'] = self.apply_mask(sample['mask'], offsets, im_size, size)
